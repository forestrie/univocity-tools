 ...
 * blob (`0x04 || x(32) || y(32)` in the `[1]` bit string). WebCrypto does
 * not import this format via `pkcs8`.
 */
function parseSec1EcPrivateKeyDer(der: Uint8Array): { x: Hex; y: Hex } {
  for (let i = der.length - 65; i >= 0; i--) {
    ...
    if (x.length === 32 && y.length === 32) {
      return {
        x: `0x${Buffer.from(x).toString("hex")}` as Hex,
        y: `0x${Buffer.from(y).toString("hex")}` as Hex,
      };
    }
  }
 ...
 * P-256 key into its public `x`/`y` coordinates. No private scalar is used —
 * deploy only needs the public key (mirrors `load_pem_es256_coords`).
 */
export async function parseEs256Pem(pem: string): Promise<{ x: Hex; y: Hex }> {
  ...
  if (pem.includes("EC PRIVATE KEY")) {
    return parseSec1EcPrivateKeyDer(new Uint8Array(pemBodyToDer(pem)));
  }
...
async function resolveEs256Coords(
  ...
  throw new Error(
    "ES256 bootstrap requires --bootstrap-es256-pem, " +
      "--bootstrap-es256-pub (BOOTSTRAP_PUB_ES256), or both " +
      "--bootstrap-es256-x and --bootstrap-es256-y",
  );
}
...
export async function resolveBootstrapKey(
  ...
  if (input.signer === undefined || input.signer.trim().length === 0) {
    throw new Error(
      "KS256 bootstrap requires --bootstrap-ks256-signer (or a Safe via " +
        "--owner-address when --safe-publish)",
    );
  }
  return {
    alg: "ks256",
