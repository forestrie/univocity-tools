...
/**
 * Safe Transaction Service failure taxonomy: `SafeServiceUnavailableError`
 * covers transport failures and 5xx (retry / degrade gracefully — propose is
 * best-effort); `SafeProposalRejectedError` covers 4xx with the service's
 * rejection body (the request itself is wrong; retrying won't help).
 */
...
/** STS could not be reached or answered 5xx: unavailable, not a rejection. */
export class SafeServiceUnavailableError extends SafeServiceError {}

/** STS answered 4xx: the request was understood and refused. */
export class SafeProposalRejectedError extends SafeServiceError {}

/**
 * Perform an STS request, mapping failures onto the error taxonomy above.
 * `operation` names the request in error messages ("proposal", …).
 */
async function serviceRequest(
  ...
  try {
    ...
    throw new SafeServiceUnavailableError(
      `Safe Transaction Service unreachable (${operation}): ` +
  ...
  if (response.status >= 500) {
    throw new SafeServiceUnavailableError(
      `Safe Transaction Service unavailable (${response.status}) during ` +
        `${operation}: ${detail || response.statusText}`,
      { status: response.status, detail },
    );
  }
  throw new SafeProposalRejectedError(
    `Safe Transaction Service rejected ${operation} (${response.status}): ` +
...
export async function fetchSafeTransaction(
  ...
  const response = await serviceRequest(url, undefined, "transaction fetch");
...
/** Abort-aware sleep; resolves after `ms` or rejects with the abort reason. */
...
function abortReason(signal: AbortSignal | undefined): unknown {
  return signal?.reason ?? new Error("delay aborted");
}

export type WaitForSafeConfirmationsOptions = {
  attempts?: number;
  delayMs?: number;
  delay?: DelayFn;
  signal?: AbortSignal;
...
export async function waitForSafeConfirmations(
  ...
  safeTxHash: Hex,
  confirmationsRequired: number,
  options?: WaitForSafeConfirmationsOptions,
): Promise<SafeServiceTransaction> {
